...
public class AddBankClient extends BaseHttpClient {

    public HttpReqResCallBack callBack;

    public AddBankClient(Context context) {
        super.context = context;
    }

    public void getJsonForType(Map<String, String> mapOfRequestParams) {
        HttpClient.context = context;
        HttpClient.post(UrlBuilder.formatRequestURLByRequestType(Constants.ADD_BANK, mapOfRequestParams), mapOfRequestParams, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                if (responseBody != null) {
                    callBack.jsonResponseReceived(new String(responseBody), statusCode, Constants.ADD_BANK);
                } else {
                    callBack.jsonResponseReceived(null, statusCode, Constants.ADD_BANK);
                }
            }
        ...
        }, Constants.CONTENT_TYPE_JSON, Constants.ADD_BANK);
    }
}

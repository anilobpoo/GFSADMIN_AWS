...
public class GetGroupClient extends BaseHttpClient {

    public HttpReqResCallBack callBack;


    public GetGroupClient(Context context) {
        super.context = context;
    }

    public void getJsonForType(Map<String, String> mapOfRequestParams) {
        HttpClient.context = context;
        HttpClient.post(UrlBuilder.formatRequestURLByRequestType(Constants.GET_GROUP, mapOfRequestParams), mapOfRequestParams, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                if (responseBody != null) {
                    callBack.jsonResponseReceived(new String(responseBody), statusCode, Constants.GET_GROUP);
                } else {
                    callBack.jsonResponseReceived(null, statusCode, Constants.GET_GROUP);
                }
            }
        ...
        }, Constants.CONTENT_TYPE_JSON, Constants.GET_GROUP);
    }
}

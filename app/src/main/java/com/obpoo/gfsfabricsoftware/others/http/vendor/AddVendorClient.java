...
public class AddVendorClient extends BaseHttpClient {

    public HttpReqResCallBack callBack;


    public AddVendorClient(Context context) {
        super.context = context;
    }

    public void getJsonForType(Map<String, String> mapOfRequestParams) {
        HttpClient.context = context;
        HttpClient.post(UrlBuilder.formatRequestURLByRequestType(Constants.VENDOR_TYPE, mapOfRequestParams), mapOfRequestParams, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                if (responseBody != null) {
                    callBack.jsonResponseReceived(new String(responseBody), statusCode, Constants.VENDOR_TYPE);
                } else {
                    callBack.jsonResponseReceived(null, statusCode, Constants.VENDOR_TYPE);
                }
            }
        ...
        }, Constants.CONTENT_TYPE_JSON, Constants.VENDOR_TYPE);
    }
}

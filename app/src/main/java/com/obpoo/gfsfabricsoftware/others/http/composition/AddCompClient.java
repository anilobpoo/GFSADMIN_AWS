...
public class AddCompClient extends BaseHttpClient {

    public HttpReqResCallBack callBack;

    public AddCompClient(Context context) {
        super.context = context;
    }

    public void getJsonForType(Map<String, String> mapOfRequestParams) {
        HttpClient.context = context;
        HttpClient.post(UrlBuilder.formatRequestURLByRequestType(Constants.ADD_COMP, mapOfRequestParams), mapOfRequestParams, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                if (responseBody != null) {
                    callBack.jsonResponseReceived(new String(responseBody), statusCode, Constants.ADD_COMP);
                } else {
                    callBack.jsonResponseReceived(null, statusCode, Constants.ADD_COMP);
                }
            }
        ...
        }, Constants.CONTENT_TYPE_JSON, Constants.ADD_COMP);
    }
}
